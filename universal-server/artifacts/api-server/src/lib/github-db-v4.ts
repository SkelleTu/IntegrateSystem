...
function cfg() {
  ...
  if (!token || !repo || !enc) {
    if (process.env.UNIVERSAL_SERVER_LOCAL_MODE === 'true') {
      return null as any;
    }
...
async function gh(path: string, init: RequestInit = {}): Promise<Response> {
  const c = cfg();
  if (!c) throw new Error("GitHub API is not available in local mode");
  return fetch(`${API}${path}`, { ...init, headers: {
    Authorization: `Bearer ${c.token}`, Accept: "application/vnd.github+json", "X-GitHub-Api-Version": "2022-11-28", "Content-Type": "application/json", ...(init.headers ?? {})
  }});
}
...
export async function initGitHubDatabase(): Promise<void> {
  ...
  if (!c) {
    state = empty();
    dirty = false;
    logger.info({ mode: 'local-memory' }, "GitHub database skipped; using local in-memory database");
    return;
  }
...
export function scheduleGitHubPersist(): void { const c = cfg(); if (!c) return; dirty = true; if (timer) return; timer = setTimeout(() => { timer = null; void persistNow(); }, DEBOUNCE_MS); }
export async function flushGitHubDatabase(reason = "manual"): Promise<void> { const c = cfg(); if (!c) { logger.info({ reason }, "Skipping GitHub database flush in local mode"); return; } if (timer) { clearTimeout(timer); timer = null; } logger.info({ reason }, "Flushing GitHub database"); await persistNow(); }
