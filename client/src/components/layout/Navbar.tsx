  ...
  return (
    <nav className="fixed top-4 left-1/2 -translate-x-1/2 z-[100] w-[98%] max-w-[1700px] bg-black/60 backdrop-blur-xl border border-white/10 rounded-2xl px-3 md:px-5 h-14 flex items-center justify-between shadow-2xl">
      <div className="flex items-center gap-6 overflow-hidden flex-1">
        ...
        <div className="hidden lg:flex items-center gap-0.5 overflow-x-auto no-scrollbar mask-fade-right pr-2 flex-1">
          {filteredItems.map((item) => (
            <Button
              key={item.url}
              variant="ghost"
              className="text-zinc-400 hover:text-primary hover:bg-primary/10 px-2.5 h-9 flex items-center gap-1.5 font-bold uppercase italic text-[9px] tracking-tight transition-all shrink-0 no-default-hover-elevate"
