...
import { Database, User as UserIcon, Clock, Calendar, ArrowUpRight, RefreshCw, Monitor } from "lucide-react";
  ...
  return (
    <div className="fixed bottom-0 left-0 w-full h-8 bg-black/80 backdrop-blur-md border-t border-white/10 z-[10000] px-2 md:px-4 flex items-center justify-between text-[10px] uppercase tracking-widest font-bold text-zinc-400 select-none overflow-visible">
      <div className="flex items-center gap-3 md:gap-6 min-w-0 flex-1 overflow-hidden">
        ...
        <div className="hidden sm:flex items-center gap-2 border-l border-white/5 pl-3 md:pl-6 overflow-hidden min-w-0">
          <Database className={cn("w-3 h-3 transition-colors shrink-0", isSyncing ? "text-primary" : "text-zinc-600")} />
          <div className="flex items-center gap-2 min-w-0">
            {isSyncing ? (
              <span className="text-[9px] text-primary/80 animate-in fade-in slide-in-from-left-2 flex items-center gap-1"><RefreshCw className="w-2.5 h-2.5 animate-spin shrink-0" /><span className="hidden md:inline">SINCRONIZANDO...</span></span>
            ) : (
