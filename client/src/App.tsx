...
import CashRegisterOptionsComplete from "@/components/CashRegisterOptionsComplete";
import CashRegisterStateSyncBridge from "@/components/CashRegisterStateSyncBridge";
...
function Router(){
  ...
  useEffect(()=>{
    ...
    if(!isLoading&&!user&&!isPublicPage){setLocation("/login?redirect="+encodeURIComponent(window.location.pathname));return;}
    if(!isLoading&&user&&!user.enterpriseId&&location!=="/setup")setLocation("/setup")
  },[user,isLoading,location,setLocation]);
  if(isLoading)return <div className="min-h-screen bg-transparent flex items-center justify-center"><Loader2 className="w-10 h-10 text-primary animate-spin"/></div>;
  const isAuthPage=["/login","/register","/setup"].includes(location);
  if(isAuthPage)return <Switch><Route path="/login" component={Login}/><Route path="/register" component={LandingPage}/><Route path="/setup" component={SetupEnterprise}/><Route component={NotFound}/></Switch>;
  const isCashierPage=location==="/caixa"||location==="/financeiro"||location==="/relatorios";
  return <div className={`flex h-screen w-full ${isCashierPage?"overflow-hidden":""}`}><div className="flex flex-col flex-1 overflow-hidden relative"><main className={`flex-1 relative ${isCashierPage?"overflow-hidden":"overflow-y-auto custom-scrollbar"}`}><Switch><Route path="/" component={LandingPage}/><Route path="/quem-somos" component={AboutUs}/><Route path="/solucoes" component={Solutions}/><Route path="/casos-de-sucesso" component={SuccessCases}/><Route path="/privacy" component={PrivacyPolicy}/><Route path="/terms" component={TermsOfService}/><Route path="/blog" component={Blog}/><Route path="/blog/:id" component={Blog}/><Route path="/contato" component={Contact}/><Route path="/app" component={Home}/><Route path="/barber" component={BarberHome}/><Route path="/menu" component={DigitalMenu}/><Route path="/barber-queue" component={BarberQueue}/><Route path="/ponto" component={TimeClock}/><Route path="/caixa" component={Cashier}/><Route path="/caixa/fechar" component={CashierClose}/><Route path="/financeiro" component={Financeiro}/><Route path="/relatorios" component={Reports}/><Route path="/inventory" component={Inventory}/><Route path="/fiscal" component={FiscalConfig}/><Route path="/admin/labels" component={LabelSystem}/><Route path="/admin/monitoring" component={MasterControl}/><Route path="/cart" component={ClientCart}/><Route path="/admin/master" component={MasterControl}/><Route path="/admin/windows-app" component={WindowsAppRunner}/><Route path="/aura-windows" component={AuraWindows}/><Route path="/backup" component={Backup}/><Route component={NotFound}/></Switch></main></div></div>
}
...
function LandingNavigation(){
  ...
  const navLinks=[{href:"/",label:"Início"},{href:"/quem-somos",label:"Quem Somos"},{href:"/solucoes",label:"Soluções"},{href:"/casos-de-sucesso",label:"Impacto Aura"},{href:"/blog",label:"Insights"},{href:"/contato",label:"Contato"}];
